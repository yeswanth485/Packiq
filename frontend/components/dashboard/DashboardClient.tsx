        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            <KPICard
              ...
              value={latestRun.total_skus}
              icon={Package}
              trend={calculateTrend('total_skus')}
              delay={0}
            />
            <KPICard
              title="Latest Run Savings"
              value={latestRun.total_savings_inr}
              unit="₹"
              icon={TrendingUp}
              trend={calculateTrend('total_savings_inr')}
              delay={0.1}
            />
            <KPICard
              title="Avg Space Utilization"
              value={latestRun.avg_utilization_percent}
              unit="%"
              icon={Zap}
              trend={calculateTrend('avg_utilization_percent')}
              delay={0.2}
            />
            <KPICard
              title="CO2 Reduction"
              value={latestRun.co2_saved_kg}
              unit="kg"
              icon={Leaf}
              trend={calculateTrend('co2_saved_kg')}
              delay={0.3}
            />
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="bg-white/[0.03] border border-white/10 p-10 rounded-[40px] space-y-8">
              ...
              <div className="h-[350px]">
                <FragilityDonut data={latestRun.results_json} />
              </div>
            </div>
            ...
            <div className="bg-white/[0.03] border border-white/10 p-10 rounded-[40px] space-y-8">
              ...
              <div className="h-[350px] flex items-center justify-center">
                <UtilizationGauge value={latestRun.avg_utilization_percent} />
              </div>
            </div>
          </div>
        </>
