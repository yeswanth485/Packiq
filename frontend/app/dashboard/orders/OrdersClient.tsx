...
const OrdersClient = memo(function OrdersClient({ initialOrders, products }: { initialOrders: any[], products: any[] }) {
  ...
  const mergedOrders = useMemo(() => {
    ...
    const sessionOrders = optResults.map((r, index) => ({
      ...
      baseline_box: r.baseline_box || 'Standard',
      optimized_box: r.optimized_box || 'N/A',
      baseline_cost: r.baseline_cost || 0,
      total_cost: r.shipping_cost || r.total_cost || 0,
      baseline_box_cost: r.baseline_box_cost || 15,
      optimized_box_cost: r.optimized_box_cost || 0,
      savings: r.savings || 0,
      savings_percent: r.savings_percent || (r.baseline_cost ? (r.savings / r.baseline_cost * 100) : 0),
      void_pct_before: r.baseline_void_pct ?? r.void_pct_before ?? 40,
      void_pct_after: r.void_pct ?? r.void_pct_after ?? 0,
      match_score: r.match_score || (r.volume_util || 0) * 0.9 + 10,
