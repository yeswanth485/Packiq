...
export function findOptimalBox(product: { length_cm: number, width_cm: number, height_cm: number }) {
  // Add 2cm padding on each dimension for safety
  const neededL = product.length_cm + 2
  const neededW = product.width_cm + 2
  const neededH = product.height_cm + 2

  // Find smallest box that fits (sorted by volume ASC)
  const fitting = STANDARD_BOXES
    .filter(b => b.l >= neededL && b.w >= neededW && b.h >= neededH)
    .sort((a, b) => (a.l * a.w * a.h) - (b.l * b.w * b.h))

  return fitting[0] || STANDARD_BOXES[STANDARD_BOXES.length - 1]
}
...
export function optimizeProduct(product: any) {
  const optimizedBox = findOptimalBox(product)
  
  const origPrice = calcBoxPrice(product.weight_kg, product.length_cm, product.width_cm, product.height_cm)
  const optPrice = calcBoxPrice(product.weight_kg, optimizedBox.l, optimizedBox.w, optimizedBox.h) + optimizedBox.price_base
  
  const { savings_inr, savings_percent } = calcSavings(origPrice, optPrice)
  
  const productVol = product.length_cm * product.width_cm * product.height_cm
  const boxVol = optimizedBox.l * optimizedBox.w * optimizedBox.h
  const spaceUtilization = calcSpaceUtilization(productVol, boxVol)

  const originalBoxVol = product.length_cm * product.width_cm * product.height_cm // Simplification: using product dims as original box
